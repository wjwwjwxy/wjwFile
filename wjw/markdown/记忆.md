...
+ plugin使用
    ...
    **plugin本质上是一个类，webpack大部分都是使用plugin机制**
...
+ **浏览器垃圾回收机制**
+ **浏览器跨域**
1. 跨域是浏览器的行为，与http无关，当发生跨域时，浏览器还是发送了http请求，只是屏蔽了http的响应
2. 不是所有的请求方法，自定义的请求头都允许跨域请求（在服务端设置了Access-Control-Allow-Origin:*的前提下）
   需要服务端设置Access-Control-Allow-Headers\Access-Control-Allow-Methods头等,浏览器通过预请求来判断
3. Access-Control-Max-Age: 允许跨域请求的最大时间（单位为S）,没有超过这个时间不需要发送预请求
+ **浏览器安全**
...
1. 常用首部字段
    ...
    // Connection 浏览器想要优先使用的连接类型：keep-alive 为长连接（复用tcp连接）、close 为请求完成后就会关闭tcp连接
    // Date 创建报文时间
    // Content-Type 内容的媒体类型
    // CSP（Content-Security-Policy） 限制内容的加载
    // ......
    ~~~
2. *2XX 成功*   *3XX 重定向（302：临时跳转；301：永久跳转）*   *4XX 客户端错误*   *5XX 服务器错误*
3. TLS（HTTPS 还是通过了 HTTP 来传输信息，但是信息通过 TLS 协议进行了加密）
4. 对称加密（双方都知道私钥）和非对称加密（双方都知道公钥，使用公钥对*真正的密钥*进行加密，服务端用私钥解密获取*真正的密钥*）
5. Http2.0优势：信道复用、分帧传输、server push(通过link头实现)
...
2. 作用: 将函数作为另一个函数的返回值，并访问另一个函数的内的变量，延长变量的生命周期 ; 将函数作为实参传递给另一个函数使用, 让函数外部可以操作函数内部的数据 ; 
2. 使用场景：事件的回调，模拟类的私有方法，封装上下文变量

+ **promise**
1. promise三种状态：pending（初始值）、fulfilled（操作成功）、rejected（操作失败）
2. promise一旦新建就会立即执行
3. promise是如何解决回调地狱的问题（即层层嵌套的回调函数）
   1. promise的链式调用
   ~~~javascript
   function f1(){
...
7. function.length 是形参的个数，arguments.length 是实参的个数
